...
public class QrcXmlPaserDemo {

    // QRC文件类型 QRC、QRC_ROMA、QRC_TS
    private static final SupportFileTypeEnum FILE_TYPE = SupportFileTypeEnum.QRC_TS;
    ...
    public static void main(String[] args) {
        List<LyricEntity> lyricEntities;
        // ts.qrc使用qrcTsXmlParseByContent 普通qrc和Roam.qrc使用qrcXmlParseByContent
        if(SupportFileTypeEnum.QRC_TS.equals(FILE_TYPE)){
            lyricEntities = LyricParser.qrcTsXmlParseByContent(XML);
        } else if (SupportFileTypeEnum.QRC.equals(FILE_TYPE) || SupportFileTypeEnum.QRC_ROMA.equals(FILE_TYPE)) {
            lyricEntities = LyricParser.qrcXmlParseByContent(XML);
        } else {
            lyricEntities = Collections.emptyList();
        }
        System.out.println(LyricConverter.convertToLrc(lyricEntities));
        System.out.println(LyricConverter.convertToSrt(lyricEntities));
    }
}
